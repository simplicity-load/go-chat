...
const scaleRange = (value: number, fromMin: number, fromMax: number, toMin: number, toMax: number) =>
  Math.floor((value - fromMin) * (toMax - toMin) / (fromMax - fromMin) + toMin)
...
window.onload = function() {
  ...
  const MessageTypes: Partial<Record<string, string>> = { 'text': 'text', 'media': 'media' } as const
  ...
  const getMessageContent = (msg: Message) => {
    ...
    if (video.includes(extension)) return `<video id="videoMessage" controls src="${msg.body}">No support for video</video>`
    if (audio.includes(extension)) return `<audio id="audioMessage" controls src="${msg.body}">No support for audio</audio>`
    if (image.includes(extension)) return `<a id="imageMessage" href="${msg.body}" target="_blank"><img src="${msg.body}"/></a>`
